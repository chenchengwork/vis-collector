...
import img_camera from './img/camera.png';
	...
	render3D(){
		...
		new ThreeUtil(document.getElementById("space"))
			...
			.use(function () {
				...
				G_controls = new OrbitControls(camera,container);
			...
			.use(async function(){
                ...
                // ---- 添加web worker 处理性能问题 ----
				const addWebWorker = () => {
                    const worker = new Worker("asserts/webWorker/index.js");
                    worker.postMessage(
                        {
                            id: 1,
                            msg: 'Hello World',
                    ...
                    worker.onmessage = function (message, text) {
                        console.log(message)
                        const loader = new THREE.ObjectLoader();
                        // ObjectLoader 解析 TextGeometry失败
                        // THREE js 添加文字会出现卡顿的原因是，字体文件库较大，遍历寻找文字时间较长，造成ui线程阻塞，出现卡顿现象
                        // 想通过web worker的方式解决这个问题，但是toJSON()后，再转换成mesh对象时， 不x能找到字体库，不支持TextGeometry解析
                        // TODO Uncaught TypeError: Cannot read property 'bones' of undefined,
                        // TODO 参照 https://github.com/mrdoob/three.js/issues/9905
                        // console.log(loader.parse(message.data.textMeshJSON));
			...
			// 		}
			// 	}));
             //    G_Object3DGroup = await Promise.all(loadModelPromises).then((resp) => {
			// 		const group = new THREE.Object3D();
			...
			// 			}
			// 		].forEach(item => {
             //            const textMesh = this.mkTextMesh(item.text);
