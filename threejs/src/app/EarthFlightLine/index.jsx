...
import Coordinate2LngLat from './util/Coordinate2LngLat';
import CreateCities from './util/createCities';
...
import json_world_countries from './data/world-countries.json';

export default class EarthFlightLine extends PureComponent {
	componentDidMount(){
		this.createEarthFlightLine();
	}

	createEarthFlightLine() {
		var VAG = VAG || {};
		...
		VAG.Globe = function (container, opts) {
			...
			function init() {
				...
				earthGlobe = createMesh(geometry, img_worldMap);
			...
			function drawWorldMap() {
				...
				var boundaries = mapDrawer.drawThreeGeo(json_world_countries, radius, "sphere", {
					color: 0xCCE563,
					transparent: true,
					opacity: 0.5
				});
			...
			function drawDataSource() {
				...
				var material = new THREE.PointsMaterial({
					...
					depthWrite: false
					/*
					 depthWrite设置为false。该属性决定这个对象是否影响WebGL的深度缓存。将
